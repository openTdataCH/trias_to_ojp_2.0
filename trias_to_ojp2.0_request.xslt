<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:trias="http://www.vdv.de/trias" xmlns:siri="http://www.siri.org.uk/siri" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	<xsl:template name="trias:convertBooleanToString">
							...
							<Params>
								...
								<xsl:if test="//trias:IncludeRealtimeData">
									...
									<xsl:element name="UseRealtimeData">
																		<xsl:call-template name="trias:convertBooleanToString">
									       <xsl:with-param name="input" select="//trias:IncludeRealtimeData"/>
									</xsl:call-template>
									</xsl:element>
					...
					<xsl:if test="//trias:TripInfoRequest">
						<OJPTripInfoRequest>
							...
							<Params>
								...
								<xsl:if test="//trias:IncludeRealtimeData">
									<xsl:variable name="includeRealtimeData" select="//trias:IncludeRealtimeData"/>
									<xsl:variable name="result" select="trias:convertBooleanToString($includeRealtimeData)"/>
								...
								<PlaceRef>
									<xsl:choose>
										<xsl:when test="//trias:Destination/trias:LocationRef/trias:StopPointRef">
											<xsl:element name="siri:StopPointRef">
												<xsl:value-of select="//trias:Destination/trias:LocationRef/trias:StopPointRef"/>
											</xsl:element>
										</xsl:when>
										<xsl:otherwise>
											<xsl:message terminate="yes">Error: The condition is not met.</xsl:message>
